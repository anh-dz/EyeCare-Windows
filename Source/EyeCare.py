...
if tuple(p.name() for p in psutil.process_iter()).count("EyeCare.exe")>1:
    Notifi().show_toast("EyeCare vẫn đang chạy!")
    sys.exit()
    ...
    def run(self):
        ...
        while not start__:
            ...
            if time__about__temp[0] == 0 and time__about__temp[1] == 0 and time__about__temp[2] == 5:
                Notifi().show_toast("Chuẩn bị nghỉ ngơi thôi, nhớ lưu công việc lại nha <3")
...
#MAIN FUNTION
    def start_active(self):
        ...
        if start__:
            ...
            if tim_cout == 0 and tim_wait == 0:
                ...
                self.msg.setText("Thời gian làm việc và nghỉ ngơi phải trên 10s")
                self.msg.exec()
            elif tim_wait>3600000 and not self.isHidden():
                self.msg.setText("Cảnh báo: Thời gian giải lao quá nhiều. Khi thời gian giải lao kích hoạt, bạn sẽ không thể sử dụng máy tính")
                self.msg.exec()
                self.active()
            else: self.active()
        else:
    ...
    def call_stop(self):
        ...
        self.start_app = QAction("Bắt đầu")
...
#CHART
    def bieu_do(self, num):
        ...
        widgets.save.setText(f"Lưu (theo {text[8:]})")
        gio = QBarSet(text[8:])
        if not num: data_temp = data
        elif num==1: data_temp = [i*60 for i in data]
        else:
            gio = QBarSet("Không ngủ đúng giờ")
    ...
    def save_active(self, num):
        ...
        if name!=('', ''):
            ...
            self.msg.setText("Lưu Thành Công!")
            self.msg.exec()
...
#SETTING
    ...
    def password_edit_active(self):
        ...
        self._pass.exec()
        if self._pass._pass_check_ == _pass:
            self._change_pass = GET_PASS(self, "--Nhập mật khẩu mới--")
            self._change_pass.exec()
            _pass_temp_ = '%s' %self._change_pass._pass_check_
            if _pass_temp_ == None or _pass_temp_ == "":
                self.msg.setText("Mật khẩu không được để trống! Đổi thất bại!")
                self.msg.exec()
                return
            _change_pass_check = GET_PASS(self, "--Nhập lại mật khẩu--")
            ...
            if _pass_temp_ == _change_pass_check._pass_check_:
                ...
                while _pass_temp_ != _change_pass_check._pass_check_:
                    ...
                    _change_pass_check = GET_PASS(self, "--Nhập lại mật khẩu (chính xác)--")
                    _change_pass_check.exec()
                    if _change_pass_check._pass_check_ == None:
                        self.msg.setText("Đổi thất bại!")
                        self.msg.exec()
                        return
            _pass = '%s' %_pass_temp_
            self.msg.setText("Đổi thành công")
            self.msg.exec()
        else:
            self.wrong_password()
...
#ANOTHER
    ...
    def update_active(self):
        try:
            if urlopen('https://anh-dz.github.io/eyecare/version.html').read() != b'1\n': self.msg.setText("Phát hiện phiên bản cập nhật mới. Truy cập trang chủ để tải phiên bản mới nhất")
            else: self.msg.setText("Không có bản cập nhật nào mới")
        except: self.msg.setText("Không thể kết nối tới Sever. Vui lòng thử lại!")
        self.msg.exec()
    def areusure(self):
        box = QMessageBox()
...
#TRAY ICON
    ...
    def tray_icon(self):
        ...
        self.show_app = QAction("Hiện")
        self.hide_app = QAction("Ẩn")
        self.quit_app = QAction("Thoát")
        self.start_app = QAction("Bắt đầu")
...
write_data(user, days[len(days)-30:], data[len(data)-30:])
